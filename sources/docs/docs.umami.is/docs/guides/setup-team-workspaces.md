# Source: https://docs.umami.is/docs/guides/setup-team-workspaces

Menu

Teams

# Set up team workspaces

Copy page

Organize your analytics across teams by creating workspaces with dedicated websites, members, and roles. This guide walks through setting up a team structure for agencies, companies, or multi-project setups.

## Planning your team structure[#](https://docs.umami.is/docs/guides/setup-team-workspaces#planning-your-team-structure)

Before creating teams, decide how to organize your websites:

| Structure | Example | When to use |
| --- | --- | --- |
| **By client** | "Acme Corp", "Beta Inc" | Agencies managing multiple clients |
| **By department** | "Marketing", "Engineering", "Product" | Companies with cross-functional analytics needs |
| **By project** | "Main App", "Blog", "Documentation" | Teams managing multiple properties |

## Step 1: Create a team[#](https://docs.umami.is/docs/guides/setup-team-workspaces#step-1-create-a-team)

1. Go to **Settings** > **Teams**.
2. Click **Create team**.
3. Enter a descriptive name and click **Save**.

You are automatically assigned as **Team Owner** with full permissions.

## Step 2: Add websites to the team[#](https://docs.umami.is/docs/guides/setup-team-workspaces#step-2-add-websites-to-the-team)

1. Navigate to the team.
2. Go to **Websites** and click **Add Website**.
3. Enter the website name and domain, then click **Save**.

Websites in a team are visible to all team members based on their role. You can also transfer existing websites from your personal account to a team — see [Manage a team](https://docs.umami.is/docs/manage-a-team#transfer-a-website).

## Step 3: Invite members[#](https://docs.umami.is/docs/guides/setup-team-workspaces#step-3-invite-members)

### Using access codes[#](https://docs.umami.is/docs/guides/setup-team-workspaces#using-access-codes)

1. Navigate to the team and find the **Access code**.
2. Share the code with the person you want to invite.
3. They go to **Settings** > **Teams** > **Join team** and enter the code.

### Using email invitations (Cloud)[#](https://docs.umami.is/docs/guides/setup-team-workspaces#using-email-invitations-cloud)

On Umami Cloud, you can invite users directly by email:

1. Navigate to the team > **Members** > **Invite user**.
2. Enter their email and click **Send**.
3. They'll receive an email with a link to join.

## Step 4: Assign roles[#](https://docs.umami.is/docs/guides/setup-team-workspaces#step-4-assign-roles)

| Role | What they can do |
| --- | --- |
| **Team Owner** | Everything, including deleting the team. |
| **Team Manager** | Everything except deleting the team. |
| **Team Member** | View and manage websites, but cannot modify team settings. |
| **View Only** | View analytics data only. Cannot create or modify websites. |

Edit a member's role from the **Members** table by clicking **Edit** next to their name.

## Working with teams[#](https://docs.umami.is/docs/guides/setup-team-workspaces#working-with-teams)

### Switching between teams[#](https://docs.umami.is/docs/guides/setup-team-workspaces#switching-between-teams)

Click the profile button in the sidebar and select **Teams** to see all teams you belong to. Click on a team to switch into that workspace.

### Personal vs. team websites[#](https://docs.umami.is/docs/guides/setup-team-workspaces#personal-vs-team-websites)

Websites in your personal account are only visible to you. Websites in a team are visible to all team members. Use personal websites for testing or personal projects, and team websites for shared work.

Tips

- **Regenerate access codes** periodically by clicking the **Regenerate** button in team settings, especially after someone leaves the team.
- Use the **View Only** role for stakeholders who need to see data but shouldn't modify anything.
- Transfer websites between your personal account and teams as your organization evolves.

[PreviousAutomate reporting with the API](https://docs.umami.is/docs/guides/automate-reporting-with-api) [NextCreate a public dashboard](https://docs.umami.is/docs/guides/create-a-public-dashboard)

On this page